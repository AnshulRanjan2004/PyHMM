...
class MyHmmScaled(object):
    ...
    def __init__(self, model_name):
        ...
        self.B = self.model["B"]
        self.symbols = list(list(self.B.values())[
    ...
    def viterbi_log(self, obs):
        ...
        for t in range(1, len(obs)):
            ...
            for y in self.states:
                (prob, state) = max(
                    (vit[t - 1][y0] + self.logA[y0][y] + self.logB[y][obs[t]], y0) for y0 in self.states)
                vit[t][y] = prob
    ...
    def backward_scaled(self, obs):
        ...
        for t in reversed(range(T - 1)):
            ...
            for y in self.states:
                beta_local[y] = sum(
                    (self.bwk_scaled[t + 1][y1] * self.A[y][y1] * self.B[y1][obs[t + 1]]) for y1 in self.states)
    ...
    def forward_scaled(self, obs):
        ...
        for t in range(1, len(obs)):
            ...
            for y in self.states:
                ...
                local_alpha[y] = sum(
                    (self.fwd_scaled[t - 1][y0] * self.A[y0][y] * self.B[y][obs[t]]) for y0 in self.states)
                if local_alpha[y] == 0:
                    print("ERROR local alpha")
                    # print("ERROR local alpha is zero: y = ", y, "  y0 = ", y0)
                    # print("fwd = %3f, A = %3f, B = %3f, obs = %s" % (
                    #         self.fwd_scaled[t - 1][y0], self.A[y0][y], self.B[y][obs[t]], obs[t]))
        ...
        # NOTE: if log probability is very low, prob can turn out to be zero
